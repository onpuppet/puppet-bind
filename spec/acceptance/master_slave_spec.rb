...
describe 'bind' do
  ...
  it 'installs master with no errors' do
    pp = <<-EOS
        class { 'bind':
          key               => 'rndckey',
    ...
    expect(apply_manifest_on(master, pp, catch_failures: true).exit_code).not_to(eq(1))
    expect(apply_manifest_on(master, pp, catch_changes: true).exit_code).to(eq(0))
  end

  it 'installs slave with no errors' do
    pp = <<-EOS
          class { 'bind':
            key        => 'rndckey',
            secret     => 'Kpllul1kWrwwsnZ7VWRq5g==',
    ...
    # Run it twice and test for idempotency
    expect(apply_manifest_on(database, pp, catch_failures: true).exit_code).not_to(eq(1))
  ...
  describe 'Record Lookup' do
    ...
    it 'returns the correct IP address for static hostnames (A records)' do
    ...
    it 'returns the correct IP address for static hostnames (AAAA records)' do
      @dns_servers.each do |nameserver|
        @records.each do |record|
          if record[2] == 'AAAA'
    ...
    it 'returns the correct hostname for static IP addresses (PTR records)' do
      @dns_servers.each do |nameserver|
        @records.each do |record|
          if record[2] == 'PTR'
    ...
    it 'resolves external host names.' do
      external_hosts = [
        'www.cisco.com',
        'www.google.com',
        'www.cnn.com'
      ...
      @dns_servers.each do |nameserver|
        external_hosts.each do |host|
          ...
          expect(ip).not_to eql('Hostname not found'), "Server #{nameserver} could not resolve #{host}"
        end
      end
    end

    it 'returns the correct mail servers (MX records)' do
      @dns_servers.each do |nameserver|
        @records.each do |record|
          if record[2] == 'MX'
    ...
    it 'returns the correct nameservers (NS records)' do
      @dns_servers.each do |nameserver|
        @records.each do |record|
          if record[2] == 'NS'
    ...
    it 'returns the correct aliases (CNAME records)' do
      @dns_servers.each do |nameserver|
        @records.each do |record|
          if record[2] == 'CNAME'
